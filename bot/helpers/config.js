		...
		try {
			...
			if (config == null) {
				...
				if (Boolean(config_exists)) {
					config = await faunaClient.query(Get(Match(Index("guild_by_name"), post.guild.name))).catch(err => reject(err))
					redisClient.set(`${guild_name}_config`, JSON.stringify(config))
					resolve(config)
				} else {
					config_exists = await faunaClient.query(Exists(Match(Index('guild_name'), post.guild.name))).catch(err => reject(err))
